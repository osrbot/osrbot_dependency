...
8.0.0-osracer (2026-06-17)
--------------------------
* Add OSRacer RGB camera calibration launch file.
* Add ``--calib_yaml_path`` to write monocular calibration YAML directly to a requested path.
* Derive the default calibration ``camera_name`` from the remapped camera namespace instead of using ``narrow_stereo``.
* Resolve ``camera``, ``left_camera``, and ``right_camera`` remaps before calling ``set_camera_info``.
* Improve OpenCV display window sizing and shutdown handling.
* Convert tarfile upload path to ROS 2 service clients.
* Add local README with OSRacer usage notes.
